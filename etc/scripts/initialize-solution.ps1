...
$jobs += Start-Job -Name "DbMigrator" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/ProductManagement.DbMigrator")
    dotnet run
...
$jobs += Start-Job -Name "DevCert" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/ProductManagement.Web")
    dotnet dev-certs https -v -ep openiddict.pfx -p a49d34ed-76b1-4445-817d-4fdc39f06b29
